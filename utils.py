...
PADDING_SIZE = round(TILE_SIZE * 0.2)
...
def get_patches(img, patch_h=TILE_SIZE, patch_w=TILE_SIZE):
    ...
    if (patch_h > img.shape[0]) or (patch_w > img.shape[1]):
        print("Invalid: Crop dimensions are larger than image shapes (%r x %r with %r)" % (patch_h, patch_w, img.shape))
        exit(1)
    ...
    while y <= img.shape[0] and not y_done:
        ...
        while x <= img.shape[1] and not x_done:
            if x + patch_w > img.shape[1]:
                ...
                x_done = True
            locations.append(((y, x, y + patch_h, x + patch_w), (y + PADDING_SIZE, x + PADDING_SIZE, y + y_stride, x + x_stride),
            ...
            # patches.append(img[y:y + patch_h, input_tensor:input_tensor + patch_w, :])
            x += x_stride
        y += y_stride
...
def stitch_together(locations, patches, size, patch_h=TILE_SIZE, patch_w=TILE_SIZE):
    ...
    for location, patch in zip(locations, patches):
        ...
        output[y_paste:y_paste + height_paste, x_paste:x_paste + width_paste] = patch[y_cut:y_cut + height_paste, x_cut:x_cut + width_paste]
